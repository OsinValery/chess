...
def is_chax(board,color):
    ...
    for a in range(8):
        for b in range(8):
            if (board[a][b].figure.type == 'king') and \
                (board[a][b].figure.color == color) and (board[a][b].attacked):
                res = True
    return res
...
def able_to_do_hod(board,color):
    ...
    for a in board:
        for field in a:
            if field.figure.type not in ['empty','frozen'] and field.figure.color == color:
...
def find_fields(board,figure):
    ...
    for element in time_list:
        ...
        for a in range(8):
            for b in range(8):
                if board2[a][b].figure.type not in ['empty','frozen']:
...
def do_hod(x,y,board):
    ...
    empty = ['empty','frozen']
    if (choose_figure.type not in empty) and (choose_figure is board[x][y].figure):
        choose_figure = Figure('',0,0,'empty')
        gr_line.show_field(-1,y)
        Game.list_of_hod_field = []
        if Game.make_tips:
            delete_tips()
    elif (board[x][y].figure.type not in empty) and (board[x][y].figure.color != Game.color_do_hod_now) \
...
def after_movement(board,options):
    if Game.color_do_hod_now == 'black':
        return board
    if Game.made_moves % Game.frozen_moves != 0:
        return board
    if not Game.ind:
        return board
    kings = [1,1]
    ...
    if 0 in kings:
        ...
        print(kings)
    if kings == [0,0]:
        interfase.do_info(Get_text('game_both_frozen'))
    if kings == [1, 0]:
        interfase.do_info(Get_text('game_black_frozen'))
    if kings == [0, 1]:
        interfase.do_info(Get_text('game_white_frozen'))
    is_end_of_game(board)
    return board
...
def init_game():
    ...
    frozen_figure.get_widget(Main_Window.wid,Sizes)
