...
def move_figure(board,x,y,options=None):
    ...
    if Game.type_of_chess == 'fisher':
        if choose_figure.type == 'rook':
            Game.is_it_rocking(choose_figure,board,x)
    ...
    if choose_figure.type == 'pawn' and board[x][y].figure.pawn_on_last_line():
        if Game.type_of_chess == 'horse_battle':
            a , b = choose_figure.x, choose_figure.y
            Game.board[a][b].figure.transform_to('horse')
            choose_figure = Figure('',0,0,'empty')
...
def build_game(game):
    ...
    if game.type_of_chess in ['classic'] :
        ...
        horse_battle.init_chess(game)
    elif game.type_of_chess == 'permutation':
        permutation.init_chess(game)
    elif game.type_of_chess == 'horde':
        gen_horde.init_chess(game)
    game.do_hod = do_hod
    game.tips_drawed = False
    if game.with_time:
        game.time = Clock.schedule_interval(tick,1)
