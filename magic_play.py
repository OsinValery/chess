...
def after_movement(board, options):
    ...
    if not Game.ind:
        return board
    if Game.made_moves % Game.magia_moves == 0 and Game.color_do_hod_now == 'white':
        case = Game.case
        if Game.state_game == 'one':
            magik.magik(Game.board)
        elif options != None:
            case = options[-1]
            magik.magia_for_network_run(Game.board, case)
        else:
            magik.magia_for_network_run(Game.board, case)
        ...
        white[0] = is_chax(board2, 'white')
        if white[0]:
            white[1] = is_mate(board2, 'white')
        ...
        black[0] = is_chax(board2, 'black')
        if black[0]:
            black[1] = is_mate(board2, 'black')
