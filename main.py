    ...
    def work_message(self, message):
        ...
        if message[:4] == 'move':
            ...
            if not game.ind:
                return
            find_chess_module(
                self.type_of_chess).choose_figure = self.board[x0][y0].figure

            def movement(time_sec):
                options = data[4:]
                self.board = find_chess_module(self.type_of_chess).move_figure(
                    self.board, x1, y1, options)
            ...
            if 'offer' in data:
                ...
                def no():
                    ...
                    if self.with_time:
                        self.time = Clock.schedule_interval(
                            find_chess_module(self.type_of_chess).tick, 1)
                    self.voyaje_message = False
                ...
                def yes():
                    ...
                    self.voyaje_message = False
                    self.ind = False
                    find_chess_module(self.type_of_chess).interfase.do_info(
                        Get_text('game_draw_ok'))
