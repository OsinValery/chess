...
import rasing_figure as help_chess
...
def is_end_of_game(board):
    ...
    if Game.color_do_hod_now == 'white':
        ...
        if len(kings) > 0:
            is_end = True
            if len(kings) == 1:
                interfase.do_info(Get_text(f'game_{kings[0]}_king'))
            else:
                interfase.do_info(Get_text('game_both_king'))
    # pate
    if not is_end:
        for line in board:
            for el in line:
        ...
        for line in board:
            for field in line:
                if field.figure.type != 'empty' and \
                            field.figure.color != Game.color_do_hod_now:
